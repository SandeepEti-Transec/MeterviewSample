...
using MeterView.ScreenRendering.API.Middleware;
...
public static class WebAppServiceConfiguration
{
    ...
    {
        builder.Services.Configure<MeterViewScreenRenderingAPIOptions>(builder.Configuration.GetSection(MeterViewScreenRenderingAPIOptions.SectionName));
        ...
        builder.Services.AddCorsService("MeterView.ScreenRendering.APICorsPolicy", builder.Environment);
        builder.OpenTelemetryRegistration(builder.Configuration, "MeterView.ScreenRendering.API");
        ...
        // registers all services that inherit from your base service interface - IMeterViewScreenRenderingAPIScopedService
    ...
    private static void AddBoundaryServices(this IServiceCollection services, params Assembly[] assemblies)
    {
        ...
        foreach (var assembly in assemblies)
        {
            var rules = assembly.GetTypes()
                .Where(x => !x.IsAbstract && x.IsClass && x.GetInterface(nameof(IMeterViewScreenRenderingAPIScopedService)) == typeof(IMeterViewScreenRenderingAPIScopedService));
