...
using MeterView.Reporting.API.Databases;
using MeterView.Reporting.API.Resources;
using MeterView.Reporting.API.Services;
using MeterView.Reporting.API.Resources.HangfireUtilities;
using Resources;
using Hangfire;
...
public static class ServiceRegistration
{
    public static void AddInfrastructure(this IServiceCollection services, IWebHostEnvironment env, IConfiguration configuration)
    {
        ...
        var connectionString = configuration.GetConnectionStringOptions().MeterViewReportingAPI;
        if(string.IsNullOrWhiteSpace(connectionString))
        {
            // this makes local migrations easier to manage. feel free to refactor if desired.
            connectionString = env.IsDevelopment() 
                ? "Host=localhost;Port=55034;Database=dev_meterview.reporting.api;Username=postgres;Password=postgres"
        ...
        services.AddHostedService<MigrationHostedService<ReportingDbContext>>();
...
public static class HangfireConfig
{
    public static void SetupHangfire(this IServiceCollection services, IWebHostEnvironment env)
    {
        ...
        services.AddHangfireServer(options =>
        {
            ...
            options.ServerName = $"MeterViewReportingAPI-{env.EnvironmentName}";
