...
using MeterView.Channels.API.Middleware;
...
public static class WebAppServiceConfiguration
{
    ...
    {
        builder.Services.Configure<MeterViewChannelsAPIOptions>(builder.Configuration.GetSection(MeterViewChannelsAPIOptions.SectionName));
        ...
        builder.Services.AddCorsService("MeterView.Channels.APICorsPolicy", builder.Environment);
        builder.OpenTelemetryRegistration(builder.Configuration, "MeterView.Channels.API");
        ...
        // registers all services that inherit from your base service interface - IMeterViewChannelsAPIScopedService
    ...
    private static void AddBoundaryServices(this IServiceCollection services, params Assembly[] assemblies)
    {
        ...
        foreach (var assembly in assemblies)
        {
            var rules = assembly.GetTypes()
                .Where(x => !x.IsAbstract && x.IsClass && x.GetInterface(nameof(IMeterViewChannelsAPIScopedService)) == typeof(IMeterViewChannelsAPIScopedService));
