...
using MeterView.Identity.API.Middleware;
...
public static class WebAppServiceConfiguration
{
    ...
    {
        builder.Services.Configure<MeterViewIdentityAPIOptions>(builder.Configuration.GetSection(MeterViewIdentityAPIOptions.SectionName));
        ...
        builder.Services.AddCorsService("MeterView.Identity.APICorsPolicy", builder.Environment);
        builder.OpenTelemetryRegistration(builder.Configuration, "MeterView.Identity.API");
        ...
        // registers all services that inherit from your base service interface - IMeterViewIdentityAPIScopedService
    ...
    private static void AddBoundaryServices(this IServiceCollection services, params Assembly[] assemblies)
    {
        ...
        foreach (var assembly in assemblies)
        {
            var rules = assembly.GetTypes()
                .Where(x => !x.IsAbstract && x.IsClass && x.GetInterface(nameof(IMeterViewIdentityAPIScopedService)) == typeof(IMeterViewIdentityAPIScopedService));
