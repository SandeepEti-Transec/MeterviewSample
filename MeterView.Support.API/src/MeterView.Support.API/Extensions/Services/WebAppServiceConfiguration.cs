...
using MeterView.Support.API.Middleware;
...
public static class WebAppServiceConfiguration
{
    ...
    {
        builder.Services.Configure<MeterViewSupportAPIOptions>(builder.Configuration.GetSection(MeterViewSupportAPIOptions.SectionName));
        ...
        builder.Services.AddCorsService("MeterView.Support.APICorsPolicy", builder.Environment);
        builder.OpenTelemetryRegistration(builder.Configuration, "MeterView.Support.API");
        ...
        // registers all services that inherit from your base service interface - IMeterViewSupportAPIScopedService
    ...
    private static void AddBoundaryServices(this IServiceCollection services, params Assembly[] assemblies)
    {
        ...
        foreach (var assembly in assemblies)
        {
            var rules = assembly.GetTypes()
                .Where(x => !x.IsAbstract && x.IsClass && x.GetInterface(nameof(IMeterViewSupportAPIScopedService)) == typeof(IMeterViewSupportAPIScopedService));
